...
console.log("after setting all labels",this.allLabels)
    ...
    handlefresh=(data)=>{
      ...
      getNotes = (labelId) => {
          console.log('inside getNote from sidenav' + labelId)
          this.props.history.push('/getnoteslabel/'+labelId)
      }
    render() {
        let displayallLables = this.state.allLabels.map((object,index) => {
            return (
                <Button onClick={()=>this.getNotes(object.labelId)}>
                    ...
                    <Drawer variant='persistent' open={this.props.menuSelect} style={{ top: '64px' }} >
                       ...
                       <div>{displayallLables}</div>
                        ...
                        <Button className="labelcontent" onClick={this.handleArchive} >
                            <ArchiveOutlinedIcon className="labelicon"  sendstatus={this.handlefresh}  handlecolorstatus={this.handlecolor}/>
